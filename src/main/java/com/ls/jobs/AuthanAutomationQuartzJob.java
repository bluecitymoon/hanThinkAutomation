		...
		try {
			...
			if (storeDatasourceIdentity.equals("SOSO")) {
				...
				if (StringUtils.isNotBlank(componentType) && componentType.equals("storage")) {
					...
					responseVo = linggongAutomationService.grabReceivingReport(lastRunDate, now, authanJob);
				} else if (StringUtils.isNotBlank(componentType) && componentType.equals("returnedOrder")) {
					responseVo = linggongAutomationService.grabReturnedOrder(lastRunDate, now, authanJob);
				} else {
